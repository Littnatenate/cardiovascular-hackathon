...
export default function DashboardPage() {
  ...
  const handleNewSession = () => {
    toast({
      ...
      description: 'Creating new discharge session...',
    })
  }

  const handleSessionTap = (session: DischargeSession) => {
    // Determine which screen to navigate to based on session status/progress
    let targetScreen = 'Screen 3'
    switch (session.status) {
      case 'draft':
        targetScreen = 'Screen 3 (Patient Details)'
        break
      case 'in-progress':
        targetScreen = 'Screen 4 or 6 (Continue Session)'
        break
      case 'escalated':
        targetScreen = 'Screen 7 (Escalation Review)'
        break
      case 'completed':
        targetScreen = 'Screen 7 (Summary View)'
        break
    }

    toast({
      title: `Opening: ${session.patientName}`,
      description: `Navigating to ${targetScreen}`,
    })
  }
